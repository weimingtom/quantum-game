...
import quantum.gfx.DisplayList;
...
public strictfp class Planet extends GameObject {
	...
	int friendly = 0;
	int enemy = 0;
	boolean is_regrowing = false;	/** FIXME SERIALIZE THIS! **/
	...
	public void update() {
		...
		if( chain_planet != -1 )
		{
			Planet p = sim.getPlanet(chain_planet);
		...
		while (c_iter.hasNext()) {
			...
			{
				if (creature.isBorn() && !creature.isDying() && !sim.isAlly(owner, creature.getOwner()) || !sim.isAlly( creature.getOwner(), owner))
					enemy++;
			}
		}
	...
	public void renderMesh(GLCanvas canvas, float scale, float r, float g, float b, DisplayList mesh) {
		canvas.getGL().glColor3f(r, g, b);
		canvas.getGL().glPushMatrix();
		canvas.getGL().glTranslatef(pos.x, pos.y, 0);
		canvas.getGL().glScalef(radius * scale, radius * scale, 0);

		canvas.getGL().glBegin(GL.GL_LINE_STRIP);
		canvas.getGL().glNormal3f(0, 0, 0);
		mesh.render();
		canvas.getGL().glEnd();
		canvas.getGL().glPopMatrix();
	}

	public void render(GLCanvas canvas, Renderer renderer, DisplayList mesh) {
		canvas.getGL().glColor3f(0.7f, 0.7f, 1);
		canvas.getGL().glPushMatrix();
		canvas.getGL().glTranslatef(pos.x, pos.y, 0);
		canvas.getGL().glScalef(radius, radius, 0);
