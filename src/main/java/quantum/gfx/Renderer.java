...
{	
	...
	public Renderer( )
	{
		...
		circle = new DisplayList( );
		circle.begin( GL.GL_LINE_STRIP );			
		...
		Log.println( "[Renderer] created display list" );
	}
...
//			renderPass(sim, gui, canvas);			
	...
	{		
		...
		{			
			planet_timer.start();
			planet.render(canvas, this, circle);			
			...
			if( render_is_start_planet )
			{
				if( planet.isStartPlanet() )
					planet.renderMesh(canvas, 0.2f, 0, 1, 0, circle );
			}
