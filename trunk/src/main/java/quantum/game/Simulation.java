...
public strictfp class Simulation 
{
		...
		for( Command cmd: buffer )
		{
			...
			if( cmd instanceof ChainCommand )
			{
				...
				if( source.isReachablePlanet( target.getId() ) && 
					...
					source.getOwner() == id && target.getOwner() == id )
				{
					source.setChainedPlanet( target.getId() );
				}					
			}
			
			if( cmd instanceof AllyCommand )
			{
				AllyCommand a_cmd = (AllyCommand) cmd;
				int sourceClient = a_cmd.getSourceClient();
				
				Set<Integer> destAllies = allies.get(sourceClient);
				if (destAllies == null) {
					destAllies = new HashSet<Integer>();
					allies.put(sourceClient, destAllies);
				}
				
				if (a_cmd.getAllyDegree() == AllyCommand.ALLY) {
					destAllies.add(a_cmd.getDestinationClient());
				} else {
					destAllies.remove(a_cmd.getDestinationClient());
				}
			}
		}
	}
	
	public boolean isAlly(int src_client_id, int dest_client_id) 
	{
		boolean isAllied = false;
		
		Set<Integer> set = allies.get(src_client_id);
		if (set != null) 
			isAllied =  set.contains(dest_client_id);
	...
	/**
	 * set ally between src_client_id and dest_client_id
	 * @param src_client_id
	 * @param dest_client_id
	 */
	public void setAlly(int src_client_id, int dest_client_id, int ally_degree ) 
	{
		turn_cmd_buffer.add( new AllyCommand( src_client_id, dest_client_id, ally_degree ) );
	}
