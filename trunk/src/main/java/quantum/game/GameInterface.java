...
public strictfp class GameInterface implements MouseListener, MouseMotionListener, KeyListener
{
	...
	public void createTextures( )
	{
		...
		if( ally == null )
		{
			try
			{
				ally = Texture.loadTexture(  FileManager.readFile("ally.png" ) );
			}
			catch( Exception e )
			{
				Log.println( "[GameInterface] couldn't load 'ally.png'" );
				e.printStackTrace();
			}
		}
		...
		if( foe == null )
		{
			try
			{
				foe = Texture.loadTexture(  FileManager.readFile("foe.png" ) );
			}
			catch( Exception e )
			{
				Log.println( "[GameInterface] couldn't load 'foe.png'" );
				e.printStackTrace();
			}
		}
	}
...
//		gl.glLineWidth( 1 );
	...
	private void renderPlayerStats( GLCanvas canvas )
	{
		...
		{	
			...
			{												
				gl.glColor3f( 1, 1, 1 );
				if( !sim.isAlly(loop.getClient().getPlayer().getId(), clients.getIds().get(i)) )
				{
					foe.bind(0);
					Widget.renderTexturedQuad( 1, y - 3, 16, 16 );
					foe.unbind();
				}				
				else
				{
				...
				if( !sim.isAlly(clients.getIds().get(i), loop.getClient().getPlayer().getId()) )
				{
					foe.bind(0);
					Widget.renderTexturedQuad( 20, y - 3, 16, 16 );
					foe.unbind();
				}
				else				
				{
	...
	public void mouseClicked(MouseEvent e) 
	{
		if( e.getButton() == MouseEvent.BUTTON1 )
			checkAllyClick( e.getX(), e.getY() );
	}

	private void checkAllyClick( int mouse_x, int mouse_y )
	{
		if( loop.getClient().getPlayerList() == null )
			return;
		...
		float y = canvas.getHeight() - (font.getHeight() * active_players - font.getHeight()) ;
		mouse_y = canvas.getHeight() - mouse_y;
		...
		{			
			...
			if (clients.getIds().get(i) != loop.getClient().getPlayer().getId())
			{
				if( mouse_y <= y && mouse_y >= y - font.getHeight() )
				{
					SoundManager.playBuffer( "ally" );
					sim.setAlly( loop.getClient().getPlayer().getId(), clients.getIds().get(i), sim.isAlly(loop.getClient().getPlayer().getId(), clients.getIds().get(i))?AllyCommand.FOE:AllyCommand.ALLY );
					break;
				}
			}
	...
	public void mousePressed(MouseEvent e) 
	{
		if( e.getButton() == MouseEvent.BUTTON1 )
		{			
			if( ctrl_pressed && selected_planet != null && selected_planet.getOwner() == loop.getClient().getPlayer().getId() )
			{
				Planet target = sim.getPlanet( cam.getScreenToWorldX( e.getX()), cam.getScreenToWorldY( e.getY() ) );
				if( target != null && target.getOwner() == loop.getClient().getPlayer().getId() && 					
					target.isReachablePlanet( selected_planet.getId() ) )
				{
					SoundManager.playBuffer( "chain" );
				...
				{
					SoundManager.playBuffer( "unchain" );
